...
- **Audio:** HTMLAudio API (file:// kompatibel, kein XHR, läuft auf Firefox und Chromium)
...
Das Spiel verwendet eine klar definierte Zustandsmaschine mit zehn Zuständen (`GamePhase`):

```
[ START ] ──[SPACE / Klick]──► [ LEVEL_INTRO ] ──► [ PLAYING ] ⇌ [ PAUSED ]
                                                          │
                                          ┌───────────────┴────────────────┐
                                    [Outro-Trigger Z]              [Tod / Absturz]
                                          │                                 │
                                   [ LEVEL_OUTRO ]                  [ GAMEOVER ]
                                          │                                 │
                                  [ LEVEL_COMPLETE ]               [Neustart]──┐
                                   ┌──────┴──────┐                             │
                               [L1/L2]          [L3]                           │
                           [LEVEL_INTRO]  [Success-Screen]                     │
                           (nächstes Lv.)        │                             │
                                └────────────────┴─────────────────────────────┘
```

**Weitere Zustände:** `DEV_BROWSE` (freie Kamera für Entwicklung, F2 im PLAYING-Zustand), `AUDIO_MENU` (Lautstärke-Einstellungen vom Start-Screen aus).

**Start-Screen:** Spieltitel, Schwierigkeitsauswahl (Explorer / Adventurer / Enlightened), Steuerungshinweise.
**LEVEL_INTRO:** Dialogue-Bubble mit Level-spezifischem Text + Countdown vor dem Start.
**PLAYING:** Alle Spielmechaniken aktiv; HUD sichtbar.
**LEVEL_OUTRO:** Bubble nach Aktivierung des Outro-Triggers; Gefahr blendet aus.
**GAMEOVER:** Aktueller Score + Highscore; Neustart ohne Seitenreload.
...
- **Start-Leben:** abhängig vom Schwierigkeitsgrad (Explorer: 5, Adventurer: 3, Enlightened: 2) — dargestellt als Cat-Icons im HUD (max. 9 sichtbar). Siehe Abschnitt 2.13.
...
Das Spiel verwendet **HTMLAudio API** für alle Sound-Events und Musik (implementiert in `src/audio.js`). Die Web Audio API wurde bewusst nicht eingesetzt, da Firefox XHR-Anfragen auf `file://` blockiert (Begründung vollständig in Abschnitt „Bekannte Einschränkungen").

**SOUNDS-Map (31 Einträge):**

| Kategorie | Keys | Beschreibung |
|---|---|---|
| SFX — Spieler | jump, land, damage, balloon_collect, stomp_bounce, respawn | Bewegungs- und Interaktions-Sounds |
| SFX — UI | game_over, level_complete, menu_click, menu_nav, countdown_tick | Interface-Feedback und Menü-Navigation |
| SFX — Outro-Trigger | windrad, bell, l3_lever, water_drain, l1/l2/l3_outro_bubble | Levelabschluss-Sounds, jeweils levelspezifisch |
| SFX — Gegner | wasp_sting, wasp_death, wasp_buzz | Wespen-Interaktionen |
| SFX — Gefahren (Ambient) | smog_ambient, electricity_ambient, flood_ambient | Proximity-gesteuerte Umgebungsgeräusche |
| SFX — Plattformen | crumble, electro_crumble | Brüchige Plattform-Events (L1 / L2) |
| Musik | music_start, music_l1, music_l2, music_l3, music_victory | Start-Menü + ein Track pro Level + Sieg-Jingle |

**Wichtige Systeme:**

**Preload-Cache:** Alle SFX-Einträge werden beim Seitenlade-Vorgang in HTMLAudio-Elemente vorgeladen. Musik wird bei Bedarf gestreamt.

**Phasen-Hooks:** Übergänge zwischen `GamePhase`-Zuständen triggern automatisch passende Sounds. Beispiele: `GAMEOVER` → `game_over.mp3` + Game Over Screen mounten; `LEVEL_OUTRO` (L1) → `l1_outro_bubble.mp3` + Musik ausblenden; `LEVEL_COMPLETE` (L3) → Highscore speichern + Success Screen mounten.

**Wasp-Buzz-Proximity:** Ein looping HTMLAudio-Element (`wasp_buzz.mp3`) dessen Lautstärke quadratisch mit dem Abstand zur nächsten lebenden Wespe skaliert (Reichweite 350 px, Maximum 55 % des SFX-Volumes).

**Hazard-Ambient-Proximity:** Die drei levelspezifischen Umgebungsgeräusche (Smog, Strom, Flut) werden über denselben Proximity-Mechanismus gesteuert und verstärken sich mit sinkendem Abstand zum Hazard.

**Trim-Werte:** Pixabay-MP3-Dateien enthalten typischerweise 20–50 ms encoder-bedingte Stille am Dateianfang (LAME/FFmpeg Priming-Frames). Diese wird über `start`-Offsets im SOUNDS-Mapping übersprungen, um wahrnehmbare Verzögerungen zu minimieren.

**Audio-Grundsatz:**
Keine aggressiven oder erschreckenden Sounds. Alle Klänge unterstützen die „cozy danger"-Stimmung des Spiels.
...
```
src/
├── main.js                — Hauptschleife, Initialisierung, Render-Orchestrierung, Outro-Trigger-System
├── game-state.js          — Zustandsmaschine (GamePhase, GameState, DIFFICULTY-Tabelle)
├── input.js               — Tastatur-/Mauslistener
├── player.js              — Spielerfigur, Sprung, Sprite-Logik, Pfoten-Aktion
...
├── enemies.js             — Wespen-System (Patrol, Stomp, Knockback, Spawn, getNearestWaspDist)
├── background.js          — Parallax-Hintergrundsystem, Crossfade, Level-spezifische Renderer
├── audio.js               — HTMLAudio SFX + Musik; SOUNDS-Map, Preload, playSound/playMusic/fade, Proximity
├── dialogue.js            — Dialogue-Bubble-System (LEVEL_INTRO/OUTRO pro Level, Override-Tabellen)
├── start-screen.js        — Start-Screen React-Overlay (FlatLayoutB, Schwierigkeitsauswahl)
├── gameover-screen.js     — Game Over React-Overlay (Score, Highscore, Neustart)
├── success-screen.js      — Success/Congrats React-Overlay mit Konfetti-Animation (L3 Ziel)
└── levelcomplete-screen.js — Level-Complete React-Overlay (L1/L2 Ziel, Score-Anzeige)
```
